 ...
 *  DoBranch -> Condition KW_THEN StatementList ( KW_BREAK | KW_LOOP )
...
public class Parser {
    ...
    public interface ParseVoid {
        void parseBody();
    }

    /**
     * Parse the nonterminal name rule, that starts with expected, in a 
     ...
     * @param rule - the name of the nonterminal being recognised
     * @param expected - the start set for the nonterminal
     * @param recoverSet - the syntax error recovery set
     * @param parser - function that parses the nonterminal
     */
     ...
     * @param <Node> type of the tree node returned by the parser
     */
    public interface ParseNonterminal<Node> {
        Node parseBody();
    }

    /** Class ParserMethod provides generic methods for parsing a nonterminal
    ...
    class ParseMethod<Node> {
        ...
        ErrorReturn<Node> errorNode;
        /** The constructor takes a function to construct an error node 
         * of type Node */
        public ParseMethod(ErrorReturn<Node> errorNode) {
            this.errorNode = errorNode;
        }
		/**
         * Parse the nonterminal name rule, that starts with expected, 
     ...
     */
    ParseMethod<ExpNode> exp = new ParseMethod<ExpNode>(
            (Location loc) -> new ExpNode.ErrorNode(loc));
    ParseMethod<List<ExpNode>> expList = new ParseMethod<List<ExpNode>>(
    ...
    private ExpNode parseRelCondition(TokenSet recoverSet) {
        return exp.parse("Condition", RELCONDITION_START_SET, recoverSet,
            () -> {
                /* The current token is in RELCONDITION_START_SET */
     ...
     */
    ParseMethod<Operator> op = new ParseMethod<Operator>(
     ...
     * Note that sub-components of statements may not themselves be 
     * statements and hence may return some other type of node.
     */
    ParseMethod<StatementNode> stmt = new ParseMethod<StatementNode>(
    ...
    private StatementNode parseAssignmentList(TokenSet recoverSet) {
        return stmt.parse("Assignment", LVALUE_START_SET, recoverSet,
            () -> {
                /* The current token is in LVALUE_START_SET.
                 ...
                 */
            	Location loc = tokens.getLocation();
            	StatementNode.AssignmentListNode node = new StatementNode.AssignmentListNode(loc);
                ExpNode left = parseLValue(
                        recoverSet.union(Token.COMMA, Token.ASSIGN, Token.EQUALS));
                node.addLValue(left);
                ...
                ExpNode right = parseCondition(recoverSet.union(Token.COMMA));
                node.addExp(right);
                while(tokens.isMatch(Token.COMMA)) {
                	tokens.match(Token.COMMA);
    ...
    private StatementNode parseAssignment(TokenSet recoverSet) {
        return stmt.parse("Assignment", LVALUE_START_SET, recoverSet,
            () -> {
                 ...
                 */
            	Location loc = tokens.getLocation();
                List<ExpNode> lValues = parseLValueList(recoverSet.union(Token.COMMA, Token.ASSIGN, Token.EQUALS));
                tokens.match(Token.ASSIGN, CONDITION_START_SET);
                List<ExpNode> exps = parseConditionList(recoverSet.union(Token.COMMA));
    ...
    private List<ExpNode> parseLValueList(TokenSet recoverSet) {
        return expList.parse("LValueList", LVALUE_START_SET, recoverSet,
            () -> {
    ...
    private List<ExpNode> parseConditionList(TokenSet recoverSet) {
        return expList.parse("ConditionList", CONDITION_START_SET, recoverSet,
            () -> {
    ...
    private StatementNode parseDoStatement(TokenSet recoverSet) {
        return stmt.parse("Do Statement", Token.KW_DO, recoverSet,
            () -> {
                /* The current token is KW_DO */            	
                tokens.match(Token.KW_DO); /* cannot fail */        
                Location loc = tokens.getLocation();
                StatementNode.DoStatementNode branches = 
                        new StatementNode.DoStatementNode(loc);      
                StatementNode branch = parseDoBranch(recoverSet.union(Token.SEPARATOR, Token.KW_OD));
                branches.addBranch((StatementNode.DoBranchNode)branch);
                while(tokens.isMatch(Token.SEPARATOR)) {
                	tokens.match(Token.SEPARATOR);
                	branch = parseDoBranch(recoverSet.union(Token.SEPARATOR, Token.KW_OD));
                	branches.addBranch((StatementNode.DoBranchNode)branch);
                }
...
//                        parseStatementList(recoverSet.union(Token.KW_BREAK, Token.KW_LOOP));
//                if(tokens.isMatch(Token.KW_BREAK)) {
//                	tokens.match(Token.KW_BREAK);
//                	flag = 0;
//                } else if (tokens.isMatch(Token.KW_LOOP)) {
//                	tokens.match(Token.KW_LOOP);
//                	flag = 1;
//                } 
//                StatementNode branch = new StatementNode.DoBranchNode(loc, cond, statements, flag);
//                branches.addBranch((StatementNode.DoBranchNode)branch);
//                while(tokens.isMatch(Token.SEPARATOR)) {
//                	tokens.match(Token.SEPARATOR);
//                	flag = -1;
...
//                    	flag = 1;
//                    } 
//                    branch = new StatementNode.DoBranchNode(loc, cond, statements, flag);
//                    branches.addBranch((StatementNode.DoBranchNode)branch);
//                }
//                
//                tokens.match(Token.KW_OD, recoverSet);
...
//            });
//    }
    /** Rule: DoBranch -> Condition KW_THEN StatementList ( KW_BREAK | KW_LOOP ) */
    private StatementNode parseDoBranch(TokenSet recoverSet) {
        return stmt.parse("Do Branch", CONDITION_START_SET, recoverSet,
            () -> {
                /* The current token is KW_DO */
            	int flag = -1;
                Location loc = tokens.getLocation();
                ...
                return new StatementNode.DoBranchNode(loc, cond, statements, flag);
            });
    }
    ...
    ParseMethod<DeclNode.ProcedureNode> proc = 
            new ParseMethod<DeclNode.ProcedureNode>(
                    (Location loc) -> null);
    ...
    private DeclNode.ProcedureNode parseProgram(TokenSet recoverSet) {
        return proc.parse("Program", BLOCK_START_SET, recoverSet,
            () -> {
                ...
                proc.setLocalScope(currentScope);
    ...
    ParseMethod<StatementNode.BlockNode> block = 
        new ParseMethod<StatementNode.BlockNode>(
    ...
    ParseMethod<DeclNode.DeclListNode> declList = 
        new ParseMethod<DeclNode.DeclListNode>(
            (Location loc) -> null);
    /** RULE:
     *  Declaration -> ConstDefList | TypeDefList | VarDeclList | ProcedureDef */
    ...
    ParseMethod<ConstExp> constExp = 
        new ParseMethod<ConstExp>(
    ...
    ParseMethod<Type> type = 
        new ParseMethod<Type>(
            (Location loc) -> Type.ERROR_TYPE);
    ...
    private Type parseTypeIdentifier(TokenSet recoverSet) {
        return type.parse("Type Identifier", Token.IDENTIFIER, recoverSet,
            () -> {
                ...
                Location loc = tokens.getLocation();
                tokens.match(Token.IDENTIFIER);    /* cannot fail */
                return new Type.IdRefType(name, currentScope, loc);
            });
    }
    ...
    ParseMethod<SymEntry.ProcedureEntry> procEntry = 
        new ParseMethod<SymEntry.ProcedureEntry>(
            (Location loc) -> null);
