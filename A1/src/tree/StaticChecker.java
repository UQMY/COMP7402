...
public class StaticChecker implements DeclVisitor, StatementVisitor, 
    ...
    public void visitAssignmentListNode(StatementNode.AssignmentListNode node) {
        ...
        if(node.getLValues().size() > node.getExps().size()) {
        	staticError("too few expressions " , node.getLocation());
        }else if(node.getLValues().size() < node.getExps().size()) {
        	staticError("too few lvalues " , node.getLocation());
        }else {
        	for(int i = 0; i < node.getLValues().size(); i++) {	
	            ExpNode left = node.getLValues().get(i).transform(this);
	            if(set.contains(left.toString())) {
	        		staticError("repeated variable on left", left.getLocation());
	        	}else {
	        		set.add(left.toString());
	        	}
	            node.setVariable(left, i);
	            // Check the right side expression.
	            ExpNode exp = node.getExp(i).transform(this);
	            node.setExp(exp, i);
	            ...
	            if(! (lvalType instanceof Type.ReferenceType)) {
	                ...
	                node.setExp(baseType.coerceExp(exp), i);
	            }
	        }
        }
        endCheck("Assignment");
    ...
    public void visitAssignmentNode(StatementNode.AssignmentNode node) {
        ...
        if(node.getLValues().size() > node.getExps().size()) {
        	...
        	for(int i = 0; i < node.getLValues().size(); i++) {	
	            ...
	            node.getLValues().set(i, left);
	            // Check the right side expression.
	            ExpNode exp = node.getExps().get(i).transform(this);
	            node.getExps().set(i, exp);
	            // Validate that it is a true left value and not a constant.
	            Type lvalType = left.getType();
	            if(! (lvalType instanceof Type.ReferenceType)) {
	                if(lvalType != Type.ERROR_TYPE) {
	                 ...
	                 * type of the left side LValue. */
	                Type baseType = ((Type.ReferenceType)lvalType).getBaseType();
	                node.getExps().set(i, baseType.coerceExp(exp));
	            }
	        }
        }
        endCheck("Assignment");
    ...
    public void visitDoStatementNode(StatementNode.DoStatementNode node) {
        beginCheck("Do");
        for(DoBranchNode branch: node.getBranches()) {
        	branch.accept(this);
        }
        endCheck("Do");
    }
    ...
    public void visitDoBranchNode(StatementNode.DoBranchNode node) {
        ...
        // Check the condition.
        node.setCondition(checkCondition(node.getCondition()));
        node.getStmt().accept(this);  // Check the 'then' part
        endCheck("DoBranch");
    }
    /*************************************************
