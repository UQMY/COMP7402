...
public class CodeGenerator implements DeclVisitor, StatementTransform<Code>,
    ...
    public Code visitCallNode(StatementNode.CallNode node) {
        ...
        Code code = new Code();
        int numActualParams = node.getActualParamList().size();
        int numResultParams = proc.getType().getFormalResultParams().size();
        int numValuaParams = numActualParams - numResultParams;
        // Generate code for actual parameters which should be value parameters.
        for(int i = 0; i < numValuaParams; i++) {  
        	ExpNode exp = node.getActualParamList().get(i);
        	Code expCode = exp.genCode(this);
        	code.append(expCode);
        } 
        // Generate code for actual parameters which should be result parameters.
        for(int i = numActualParams - 1; i >= numValuaParams; i--) {  
        	ExpNode exp = node.getActualParamList().get(i);
        	Code expCode = exp.genCode(this);
        	code.append(expCode);
        } 
        ...
        /* Assign the values of the local result parameters 
         * to the corresponding actual parameter LValues.
         */
        for (int i = 0; i < numResultParams; i++ ) {
        	ExpNode exp = node.getActualParamList().get(numValuaParams + i);
        	Code expCode = exp.genCode(this);
        	// Check bound for when actual parameter LValues are subrangeType.
        	Type type = exp.getType().optDereferenceType();
        	if( type instanceof Type.SubrangeType){
        		code.genBoundsCheck(
        				((ScalarType) type).getLower(), ((ScalarType) type).getUpper());
        	}
        	code.append(expCode);
        	code.generateOp(Operation.STORE_FRAME);
        }
        /* Deallocate the space used by the value parameters.
         */
        code.genDeallocStack(numValuaParams);
        endGen("Call");
        return code;
    }
