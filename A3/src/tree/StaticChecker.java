...
import syms.Type.ProcedureType;
...
public class StaticChecker implements DeclVisitor, StatementVisitor, 
    ...
    public void visitCallNode(StatementNode.CallNode node) {
        ...
        ProcedureType type = procEntry.getType();
        List<SymEntry.ParamEntry> formalParams = type.getFormalParams();
        List<SymEntry.ParamEntry> formalResultParams = type.getFormalResultParams();
        List<ExpNode> actualParamList = node.getActualParamList();
        // Check the number of parameters
        if(actualParamList.size() != (formalParams.size() + formalResultParams.size())) {
        	staticError("wrong number of parameters", node.getLocation());
        	endCheck("Call");
            return;
        }
        // Check formal value parameters
        for(int i = 0; i < formalParams.size(); i++) {
        	SymEntry.ParamEntry paramEntry = formalParams.get(i);
        	paramEntry.setOffset(-(actualParamList.size() - i));
        	ReferenceType paramType = paramEntry.getType();
        	ExpNode exp = actualParamList.get(i).transform(this);
        	actualParamList.set(i, paramType.optDereferenceType().coerceExp(exp));
        }
        // Check result value parameters
        for(int i = 0; i < formalResultParams.size(); i++) { 	
        	SymEntry.ParamEntry paramEntry = formalResultParams.get(i);
        	paramEntry.setOffset(-(1 + i));
        	int idx = i + formalParams.size();
        	ExpNode exp = actualParamList.get(idx).transform(this);
        	// check whether the result parameter is LValue
        	if (!(exp instanceof VariableNode)) {
        		staticError("actual result parameter must be an LValue", exp.getLocation());
        	}
        	exp.getType().optDereferenceType().coerceExp(
        			new VariableNode(exp.getLocation(), paramEntry));
        	actualParamList.set(idx, exp);
        }
        endCheck("Call");
    }
