...
public abstract class Type 
{
    ...
    protected Type(Location loc, int space, boolean resolved) {
        this.loc = loc;
        this.space = space;
        this.resolved = resolved;
    }
    /** Constructor when the type is not initially resolved. */
    protected Type(Location loc, int space) {
        this(loc, space, false);
    }
    protected Type(Location loc, int space, boolean resolved, String name) {
    ...
    public ExpNode coerceToType(ExpNode exp) throws IncompatibleTypes {
         ...
         * fromType is ERROR_TYPE and hence an error message already given.
         */
    ...
    /** VOID_TYPE
     * Void type used for return type of a procedure (i.e. no return type. */
    public static final Type VOID_TYPE = 
            new Type(ErrorHandler.NO_LOCATION, 0, true, "void") {       
    };
    ...
    public static class ScalarType extends Type {
        ...
        public ScalarType(Location loc, String name, int size, int lower, int upper) {
    ...
    public static class SubrangeType extends ScalarType {
        ...
        public SubrangeType(Location loc, ConstExp lowerExp, ConstExp upperExp) {
             ...
             */
            super(loc, StackMachine.SIZE_OF_INT);
            ...
            if(exp instanceof ExpNode.ArgumentsNode) {
                ...
                if(this.getTypes().size() == args.getArgs().size()) {
                    ...
                    for(Type t : this.getTypes()) {
                         ...
                         * which we pass back up to the next level
                         */
                        try {
                     ...
                     * will be of type this ProductType. We return an 
    ...
    public static class FunctionType extends Type {
        ...
        public FunctionType(Location loc, Type arg, Type result) {
            super(loc, 0);
            this.argType = arg;
    ...
    public static class IntersectionType extends Type {
        ...
        public IntersectionType(Location loc, Type... typeArray) {
            super(loc, 0);
    ...
    public static class ProcedureType extends Type {
        public ProcedureType(Location loc) {
            ...
            super(loc, 2*StackMachine.SIZE_OF_ADDRESS); 
        }
    ...
    public static class AddressType extends Type {
        ...
        public AddressType(Location loc, Type baseType) {
            super(loc, StackMachine.SIZE_OF_ADDRESS);
    ...
    public static class ArrayType extends FunctionType {
        ...
        public ArrayType(Location loc, Type indexType, Type elementType) {
            super(loc, indexType, elementType);           // Type size will need to be resolved later
            this.resolved = false;
        }
        @Override
        public ArrayType resolveType() {
            super.resolveType();
            /* Calculate the size of the whole array */
            if(argType instanceof SubrangeType 
                || argType instanceof EnumerationType
            ) {
                ScalarType range = (ScalarType)argType;
                space = resultType.getSpace() * 
                        (1 + range.getUpper() - range.getLower());
            } else if(argType != ERROR_TYPE) {
                errors.error("index type of an array must be a subrange", loc);
            }
            return this;
        }
        @Override
    ...
    public static class EnumerationElement {
        /** Name of the element of the enumeration */
        String ident;
        /** Location of declaration of element */
        Location loc;
        
        public EnumerationElement(String ident, Location loc) {
            this.ident = ident;
            this.loc = loc;
        }
    }
    /** An enumeration is a scalar type */
    public static class EnumerationType extends ScalarType {
        /** List of elements in the enumeration */
        private List<EnumerationElement> elements;
        /** Static level of the enumeration */
        private int staticLevel;
        /** Scope in which enumeration declared */
        private Scope scope;
        
        public EnumerationType(Location loc, List<EnumerationElement> elements, Scope scope) {
            // Enumerations take one word and initially the range
            // of values is empty, represented by [0..-1]
            super(loc, 1, 0, -1);
            this.elements = elements;
            this.scope = scope;
            this.staticLevel = scope.getLevel();
            for(EnumerationElement entry : elements) {
                this.upper++;
                if(scope.addConstant(entry.ident, entry.loc, this, upper) 
                        == null) {
                    errors.error(entry.ident + " already declared", 
                            entry.loc);
                }
            }
        }
        public void addOperators() {
            ...
            scope.addOperator("_=_", loc, relOpType);
            scope.addOperator("_!=_", loc, relOpType);
            scope.addOperator("_<_", loc, relOpType);
            scope.addOperator("_>_", loc, relOpType);
            scope.addOperator("_<=_", loc, relOpType);
            scope.addOperator("_>=_", loc, relOpType);
            Type.FunctionType enumOpType =
                new Type.FunctionType(this, this);
            scope.addOperator("pred_", loc, enumOpType);
            scope.addOperator("succ_", loc, enumOpType);
        }
        @Override
        public String toString() {
            String s = "enumeration(" + staticLevel + ") {";
            String sep = "";
            for(EnumerationElement entry : elements) {
                s += sep + entry.ident;
                sep = ",";
