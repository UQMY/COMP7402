...
public class Scope {
    ...
    private SymEntry.ProcedureEntry ownerEntry;
    ...
    public Scope extendCurrentScope() {
        Scope extendedScope = new Scope(this,   level, ownerEntry);
        extendedScope.extension = true;
        return extendedScope;
    }
    /** Lookup the identifier in the current scope only,
     * not any ancestor scopes.
     */
    public SymEntry lookupLocal(String id) {
    ...
    public int allocVariableSpace(int size) {
        if(extension) {
            //System.out.println("Allocating in extension " + this);
